...
public function processLogin(Request $request) {
    // Fetch data required for the dashboard
    ...
    if ($request->role == "mahasiswa") {
        if (auth()->guard('mahasiswa')->attempt(['nim' => $request->username, 'password' => $request->password])) {
            ...
            if (auth()->guard('mahasiswa')->user()->role == 1) {
                return redirect()->route('dashboard2')->with('success', 'Login berhasil');
                ...
                return redirect()->route('dashboard3')->with('success', 'Anda Berhasil Login');
            }
        } else {
